...
static volatile TickType_t g_last_packet_tick = 0;
...
static void sniffer_enqueue_packet(const ble_packet_t* packet) {
  ...
  memcpy(&g_packet_buffer.packets[g_packet_buffer.head], packet,
         sizeof(ble_packet_t));
...
static int sniffer_dequeue_packet(ble_packet_t* packet) {
  ...
  if (g_packet_buffer.count > 0) {
    memcpy(packet, &g_packet_buffer.packets[g_packet_buffer.tail],
           sizeof(ble_packet_t));
    g_packet_buffer.tail = (g_packet_buffer.tail + 1) % SNIFFER_BUFFER_SIZE;
    g_packet_buffer.count--;
    result = 1;
...
void sniffer_send_packet_serial(const ble_packet_t* packet) {
  ...
  // Format: HEX_PREFIX|mac|rssi|battery|manufacturer|model|mfg_data_hex
  // Example: GATT|AA:BB:CC:DD:EE:FF|-65|87|Samsung|Galaxy S24|010203
  printf("GATT|%02X:%02X:%02X:%02X:%02X:%02X|%d|%d|%s|%s|", packet->mac[0],
         packet->mac[1], packet->mac[2], packet->mac[3], packet->mac[4],
  ...
  for (int i = 0; i < packet->mfg_data_len; i++) {
    printf("%02X", packet->mfg_data[i]);
  }
  printf("\r\n");
}
...
void sniffer_on_packet_received(const uint8_t* mac, int8_t rssi,
  ...
  sniffer_on_packet_received_ex(mac, rssi, channel, timestamp, payload,
                                payload_len, 0, 0);
}
...
void sniffer_on_packet_received_ex(const uint8_t* mac, int8_t rssi,
  ...
  /* payload_len is uint8_t (max 255), packet.payload is 255 bytes, so always
   * fits */
  ...
  // Update last packet tick
  g_last_packet_tick = xTaskGetTickCount();
...
const char* gatt_get_service_name(uint16_t uuid) {
  switch (uuid) {
    case GATT_SERVICE_GENERIC_ACCESS:
      return "Generic Access";
    case GATT_SERVICE_GENERIC_ATTRIBUTE:
      return "Generic Attribute";
    case GATT_SERVICE_IMMEDIATE_ALERT:
      return "Immediate Alert";
    case GATT_SERVICE_LINK_LOSS:
      return "Link Loss";
    case GATT_SERVICE_TX_POWER:
      return "TX Power";
    case GATT_SERVICE_DEVICE_INFO:
      return "Device Info";
    case GATT_SERVICE_BATTERY:
      return "Battery";
    case GATT_SERVICE_HEART_RATE:
      return "Heart Rate";
    case GATT_SERVICE_ENVIRONMENTAL:
...
void sniffer_start(void) { printf("[SNIFFER] Capture started\r\n"); }
...
void sniffer_get_stats(uint32_t* total_packets, uint32_t* overflow_count,
  ...
  taskENTER_CRITICAL();
  if (total_packets)
    *total_packets = g_packet_buffer.packet_count;
  if (overflow_count)
    *overflow_count = g_packet_buffer.overflow_count;
  if (buffer_count)
    *buffer_count = g_packet_buffer.count;
  taskEXIT_CRITICAL();
}
...
static void sniffer_task(void* arg) {
  ...
  int green_led_state = 1;  // Start OFF (active low)

  printf("[SNIFFER] Task running\r\n");

  while (1) {
    // Process packets in a loop to drain buffer faster
    int processed = 0;
    while (processed < 10 && sniffer_get_packet(&pkt)) {
      // sniffer_send_packet_serial(&pkt); // Disable redundant print to save
      // UART bandwidth
      processed++;
    }
    ...
    if (processed == 0) {
      ...
      // Yield briefly to let other tasks run if we processed a batch
      taskYIELD();
    }
    ...
    if ((now - g_last_packet_tick) < pdMS_TO_TICKS(1000)) {
      ...
      // Blink Green every 100ms
      if ((now - last_led_toggle) > pdMS_TO_TICKS(100)) {
        last_led_toggle = now;
        green_led_state = !green_led_state;
        bl_gpio_output_set(LED_GREEN, green_led_state);
      }
    } else {
      // Timeout - Green OFF, Red ON
    ...
    if ((now - last_status) > pdMS_TO_TICKS(10000)) {
      ...
      printf("[SNIFFER] Total=%lu Rate=%lu/s Overflow=%lu Buffer=%u/%u\r\n",
             g_packet_buffer.packet_count, rate, g_packet_buffer.overflow_count,
             g_packet_buffer.count, SNIFFER_BUFFER_SIZE);
