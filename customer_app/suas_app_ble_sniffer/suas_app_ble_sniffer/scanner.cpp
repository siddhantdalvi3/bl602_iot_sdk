...
static void scanner_device_found(const bt_addr_le_t* addr, int8_t rssi,
  ...
  sniffer_on_packet_received_ex(addr->a.val, rssi, g_current_channel, ts,
...
static void ble_stack_init(void) {
  ...
  // Set up scan params
  scan_param.type =
      g_scan_mode ? BT_LE_SCAN_TYPE_ACTIVE : BT_LE_SCAN_TYPE_PASSIVE;
...
void scanner_get_stats(uint32_t* adv_count, uint32_t* scan_rsp_count) {
  if (adv_count)
    *adv_count = g_adv_count;
  if (scan_rsp_count)
    *scan_rsp_count = g_scan_rsp_count;
}
